...
export default class Drawer extends React.PureComponent<Props> {
	...
	componentDidUpdate(prevProps: Props) {
		...
		if (prevProps.swipeDistanceThreshold !== swipeDistanceThreshold) {
			this.swipeDistanceThreshold.setValue(
				swipeDistanceThreshold !== undefined ? swipeDistanceThreshold : SWIPE_DISTANCE_THRESHOLD_DEFAULT,
			);
		}
	...
	private isDrawerTypeFront = new Value<Binary>(this.props.drawerType === 'front' ? TRUE : FALSE);
	...
	private drawerPosition = new Value<number>(this.props.drawerPosition === 'right' ? DIRECTION_RIGHT : DIRECTION_LEFT);
		...
		cond(
			...
			min(
				multiply(
					...
					sub(sub(this.containerWidth, this.drawerWidth), sub(this.touchX, this.gestureX)),
					DIRECTION_RIGHT,
				),
				0,
			),
	...
	private swipeVelocityThreshold = new Value<number>(this.props.swipeVelocityThreshold);
					...
					cond(
						...
						cond(
							...
							cond(
								...
								lessThan(cond(eq(this.velocityX, 0), this.gestureX, this.velocityX), 0),
							),
							this.isOpen,
						),
					),
	...
	// 	{
	// 		nativeEvent: {
	// 			oldState: (s: Animated.Value<number>) => cond(eq(s, State.ACTIVE), set(this.manuallyTriggerSpring, TRUE)),
	// 		},
	// 	},
	// ]);

	private handleContainerLayout = (e: LayoutChangeEvent) => this.containerWidth.setValue(e.nativeEvent.layout.width);
	...
	render() {
		...
		// front drawer - drawer slides over content
		// back drawer - content slides to reveal drawer
        ...
        > */}
				{/* <Animated.View */}
				<View onLayout={this.handleContainerLayout} style={styles.main}>
					<View
						style={[styles.content, sceneContainerStyle as any]}
